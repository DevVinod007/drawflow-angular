...
drop(ev: any) {
  console.log("drop", ev);
  if (ev.type === "touchend") {
    let parentdrawflow = document.elementFromPoint(
      this.mobile_last_move.touches[0].clientX,
      this.mobile_last_move.touches[0].clientY
    );

    if (parentdrawflow) {
      parentdrawflow = parentdrawflow.closest("#drawflow");
    }
...
addNodeToDrawFlow(name: string, pos_x: number, pos_y: number):void {
  ...
  switch (name) {
    ...
    case "slack":
      ...
      if (this.editor.nodes["aws"]) {
        const slackId = this.editor.getNodeIdFromName("slack");
        const awsId = this.editor.getNodeIdFromName("aws");

        if (slackId && awsId) {
          this.editor.addConnection(slackId, "output_1", awsId, "input_1");
        }
      }
      break;
      break;
